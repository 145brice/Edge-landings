...
function buildReport(facts, links, speed, requestedUrl) {
  ...
  const performancePoints = Math.round(performanceScore * 0.25);
  add('performance', 'Mobile performance', performanceScore >= 75, performancePoints, 25, speed ? `PageSpeed mobile score: ${performanceScore}/100.` : `Estimated from a ${facts.responseMs} ms server response; PageSpeed data was unavailable.`, 'Compress large media, reduce blocking scripts, and prioritize above-the-fold content.');
