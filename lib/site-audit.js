...
function buildReport(facts, links, speed, requestedUrl) {
  ...
  if (speed) {
    const performancePoints = Math.round(speed.score * 0.25);
    add('performance', 'Mobile performance', speed.score >= 75, performancePoints, 25, `PageSpeed mobile score: ${speed.score}/100.`, 'Compress large media, reduce blocking scripts, and prioritize above-the-fold content.');
  } else {
    add('performance', 'Mobile performance', null, 0, 0, 'Google PageSpeed data was unavailable, so performance is not included in this score.', 'Run a dedicated performance test before launch.');
  }
  const availableChecks = checks.filter((check) => check.max > 0);
  const earned = availableChecks.reduce((total, check) => total + check.points, 0);
  const possible = availableChecks.reduce((total, check) => total + check.max, 0);
  const score = Math.max(0, Math.min(100, Math.round((earned / possible) * 100)));
  const issues = availableChecks.filter((check) => check.points < check.max).sort((a, b) => (b.max - b.points) - (a.max - a.points)).slice(0, 5);
  ...
  return {
    ...
    scoreLabel: speed ? 'Homepage health score' : 'Homepage setup score',
    performance: speed ? { ...speed, source: 'Google PageSpeed Insights' } : { score: null, source: 'Unavailable' },
