...
async function checkHealth() {
  const { error } = await client().from('client_projects').select('id').limit(1);
  if (error) throw error;
  return true;
}

module.exports = { createProject, getProjectByTokenHash, getProjectById, getProjectsByEmail, listRequests, listAllRequests, addRequest, updateRequest, listMessages, addMessage, updateProject, listProjects, checkHealth };
