...
test('main hosts show the hub while the website subdomain keeps its own homepage', async () => {
 ...
 const read=(host,path='/')=>new Promise((resolve,reject)=>http.get({hostname:'127.0.0.1',port:server.address().port,path,headers:{Host:host}},response=>{let body='';response.on('data',chunk=>body+=chunk);response.on('end',()=>resolve({status:response.statusCode,body,headers:response.headers}));}).on('error',reject));
 try {
  ...
  for(const path of ['/pricing.html','/templates.html','/contact.html','/tax.html','/hub.css']) assert.equal((await read('www.edgelandings.com',path)).status,200);
  ...
  const leads = await read('www.edgelandings.com','/leads.html');
  assert.equal(leads.status,302);
  assert.equal(leads.headers.location,'https://leads.edgelandings.com/');
