(() => {
  ...
  function render(data) {
    ...
    const metrics = [
      ['Performance', data.performance?.score == null ? 'Not included' : `${data.performance.score}/100`],
    ...
    report.innerHTML = `<div class="report-heading"><div class="score-ring" style="--score:${data.score}"><strong>${data.score}</strong><span>/100</span></div><div><div class="eyebrow">${escape(data.scoreLabel || 'Homepage health score')}</div><h3>${escape(data.grade)}</h3><p>${escape(data.finalUrl)}</p><small>Audited ${new Date(data.auditedAt).toLocaleString()} · Preset scoring, no AI</small></div></div><section class="audit-metrics">${metrics}</section><section><h3>Top issues</h3><div class="audit-issues">${issueCards || '<p>No major preset issues were detected.</p>'}</div></section><section class="quick-wins"><h3>Quick wins</h3><ol>${wins || '<li>Keep monitoring performance and content quality.</li>'}</ol></section><aside class="report-ad"><span>Beta pricing · Half price</span><strong>Want these fixes handled for you?</strong><p>Edge Landings builds and maintains conversion-focused local business websites from $49/month during beta.</p></aside><div class="report-actions"><button class="button primary" id="download-report" type="button">Download my report</button><a class="button secondary" href="${mockupUrl}">Start a free draft with these fixes</a></div><p class="report-note">This preset checks a public small-business homepage and a limited sample of internal links. If PageSpeed data is unavailable, performance is shown as not included and does not affect the score. This is a screening report, not a comparison of a marketing site with a web platform or a compliance certification.</p>`;
    report.hidden = false;
